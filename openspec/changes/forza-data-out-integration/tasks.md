...
- **Tasks**:
  - [ ] Initialize `forza-bridge.js` in the repository root.
  - [ ] Create UDP socket listening on port `5600` (default) using `dgram`.
  - [ ] Implement an HTTP server on port `5601` with an SSE endpoint at `/telemetry`.
  - [ ] Add graceful `EADDRINUSE` port collision error handling to the HTTP server in `forza-bridge.js`.
  - [ ] Implement UDP payload parser for FH5 (312 bytes) and FH6 (324 bytes) packet formats.
  - [ ] Extract key parameters using Little-Endian reads (e.g., `CarOrdinal`, `EngineMaxRpm`, `CurrentEngineRpm`, `Speed`, `Power`, `Torque`, `DrivetrainType`, `Throttle` (Accel), `Brake`, `AccelerationZ`).
  - [ ] Broadcast parsed data to SSE clients as serialized JSON.
  - [ ] Create `launch.bat` in the repository root with robust pathing (`pushd`), Node.js availability pre-check, background browser launch, and foreground bridge execution.
...
- **Tasks**:
  - [ ] Implement `EventSource` initialization, subscription, and cleanup in a React `useEffect` hook.
  - [ ] Add the "Live Telemetry" settings panel inside the sidebar or under general settings in `index.html`.
  - [ ] Add input field for the bridge URL (default: `http://localhost:5601/telemetry`) and a Connect/Disconnect toggle button.
  - [ ] Track and display connection state (`Disconnected`, `Connecting`, `Connected`) with corresponding indicator colors (Red/Yellow/Green).
  - [ ] Render live diagnostics in the UI: current RPM, Speed, and active `CarOrdinal`.
...
- **Tasks**:
  - [ ] Embed the static lookup database (`CAR_DATABASE`) mapping key `CarOrdinal` values to stock `{ name, weight, frontBias, layout }`.
  - [ ] Implement maximum tracker for HP and Torque: `HP_peak = Math.max(HP_peak, Power / 745.7)` and `Torque_peak = Math.max(Torque_peak, Torque * 0.73756)`.
  - [ ] Implement the dynamic weight estimation calculation: `Weight = Power / (v * ax)` (converted to lbs).
  - [ ] Implement filter conditions to execute the mass equation: throttle > 80% (`throttle > 204`), brake = 0 (`brake === 0`), speed > 10 m/s, and acceleration > 0.5 m/s².
  - [ ] Implement a sliding window queue (size 80) and a median calculation helper function to filter noise out of the dynamic weight estimate.
...
- **Tasks**:
  - [ ] Add state variables for locking (`weightLocked`, `frontBiasLocked`, `layoutLocked`), defaulting to true.
  - [ ] Render lock badges (🔒/🔓) adjacent to input labels for Weight, Front Bias, and Layout.
  - [ ] Bind value inputs to live telemetry estimations/lookups when locked, and disable manual keyboard inputs for those fields.
  - [ ] Enable normal manual input editing when the lock is toggled to unlocked (🔓), ignoring subsequent telemetry updates for that field.
...
- **Tasks**:
  - [ ] Add unit tests in `tests.js` to verify:
  ...
  - [ ] Run the test suite (`node tests.js`) to guarantee no regression on core calculator physics.
  - [ ] Update `README.md` to document the new Live Telemetry feature:
