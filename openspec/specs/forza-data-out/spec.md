...
The system MUST connect to the local UDP-to-SSE bridge at `http://localhost:5601/telemetry` to receive telemetry, and it MUST display the current connection status to the user.
...
- **THEN** the system attempts to establish an SSE connection to `http://localhost:5601/telemetry` and displays a status of "Connecting"

#### Scenario: Successful connection
- **WHEN** the SSE connection to `http://localhost:5601/telemetry` is successfully established and active telemetry packets are being received
...
- **WHEN** a binary payload of 312 bytes is received via the SSE stream
...
- **WHEN** a binary payload of 324 bytes is received via the SSE stream
